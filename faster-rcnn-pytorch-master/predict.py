...
if __name__ == "__main__":
    ...
    if mode == "predict":
        ...
        '''
        annotation_path = 'F:/rcnn/faster-rcnn-pytorch-master/faster-rcnn-pytorch-master/data.txt'
        with open(annotation_path) as f:
            ...
            for i in range(0,len(image)):
                ...
                r_image = frcnn.detect_image(image_data)
                #print(r_image[0][0])
                plt.imshow(image[i], cmap='gray')
                ax = plt.gca()
                for i in range(0,len(r_image)):
                    ax.add_patch(plt.Rectangle((r_image[i][0], r_image[i][1]), r_image[i][2]-r_image[i][0], r_image[i][3]-r_image[i][1], color="blue", fill=False, linewidth=1))
                plt.show()
